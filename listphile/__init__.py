...
class FileLister:
	...
	def write_list(self, folder:ty.Union[PathOrStr,ty.Sequence[PathOrStr]] = '', list_path:PathOrStr = ''):
		...
		list_path = _parse_path(list_path)
		...
		try:
			...
			for folder in base_folders:
				abs_folder = _parse_path(folder).absolute()
	...
	def run_folder(self, folder:PathOrStr = '', *,
	...
	def generate(self, folder:PathOrStr = '', *, _item:ty.Optional[paths.PathItem] = None
	...
	def parse_list(self, list_path:PathOrStr) -> ty.Generator[ty.Tuple[str, Path, ty.Optional[format.Props]], None, None]:
...
def write_list(folder:ty.Union[PathOrStr,ty.Sequence[PathOrStr]] = '', list_path:PathOrStr = '',
	           options:ty.Optional[dict] = None):
	FileLister(**(options or {})).write_list(folder, list_path)

def generate(folder:PathOrStr = '', options:ty.Optional[dict] = None
             ) -> ty.Generator[ty.Tuple[str, Path, format.Props], None, None]:
	yield from FileLister(**(options or {})).generate(folder)

def parse_list(list_path:PathOrStr, options:ty.Optional[dict] = None
               ) -> ty.Generator[ty.Tuple[str, Path, format.Props], None, None]:
